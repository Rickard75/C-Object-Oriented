...
vector<AnalysisSteering*> AnalysisFactory::create( const AnalysisInfo* info ) {
  ...
  for (const auto& element : *fm) { //itero su tutti i factory registrati
    // create analysis object if its name is listed in the command line
    if ( info->contains( element.first ) ) // cioè se c'era la stringa nella riga di comando
...
map<string,AnalysisFactory::AbsFactory*>*
           AnalysisFactory::factoryMap() { //lo creo la prima volta, e le successive passo lo stesso
